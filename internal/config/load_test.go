...
func TestLoadParsesPluginConfig(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "policy.yaml")
	content := `
plugins:
...
`
	...
	if len(cfg.Plugins) != 1 {
		t.Fatalf("unexpected plugin count: %+v", cfg.Plugins)
	}
	if got := cfg.Plugins[0].Config["kernel_log_lookback"]; got != "5m" {
		t.Fatalf("unexpected plugin config: %+v", cfg.Plugins[0].Config)
	}
	if got := cfg.Plugins[0].Config["row_remap_fail_threshold"]; got != 2 {
		t.Fatalf("unexpected numeric plugin config: %+v", cfg.Plugins[0].Config)
	}
}
